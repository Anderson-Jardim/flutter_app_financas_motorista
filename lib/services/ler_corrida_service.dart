...
Future<ApiResponse> getlerCorridaDetail() async {
  ApiResponse apiResponse = ApiResponse();
  try {
    String token = await getToken();
    final response = await http.get(
      Uri.parse(getLerCorrida),
      headers: {
   ...
   if (response.statusCode == 200) {
      ...
      List<LerCorridaModel> lerCorridamodel = data.map((item) => LerCorridaModel.fromJson(item)).toList();

      // Definir os dados da resposta como a lista de InfooneModel
      apiResponse.data = lerCorridamodel;
    } else {
