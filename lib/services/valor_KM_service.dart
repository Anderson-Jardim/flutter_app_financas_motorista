...
import '../models/valorKM_model.dart';

Future<ApiResponse>getValorKM_Detail()async{
  ApiResponse apiResponse = ApiResponse();
  try{
    String token = await getToken();
    final response = await http.get(
      Uri.parse(valorKM_URL),
      headers: {
    ...
    if (response.statusCode == 200) {
      ...
      List<valorKM_Model> valorkm_model = data.map((item) => valorKM_Model.fromJson(item)).toList();

      // Definir os dados da resposta como a lista de InfooneModel
      apiResponse.data = valorkm_model;
    }else {
