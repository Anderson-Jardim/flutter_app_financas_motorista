...
Future<ApiResponse> getclassCorridasDetail() async {
  ApiResponse apiResponse = ApiResponse();
  try {
    String token = await getToken();
    final response = await http.get(
      Uri.parse(classcorridasURL),
      headers: {
   ...
   if (response.statusCode == 200) {
      ...
      List<classCorridasModel> classcorridasmodel = data.map((item) => classCorridasModel.fromJson(item)).toList();

      // Definir os dados da resposta como a lista de InfooneModel
      apiResponse.data = classcorridasmodel;
    } else {
