...
Future<ApiResponse> getSaidasDetail() async {
  ApiResponse apiResponse = ApiResponse();
  try {
    String token = await getToken();
    final response = await http.get(
      Uri.parse(saidaLucroGet),
      headers: {
   ...
   if (response.statusCode == 200) {
      ...
      List<saidaLucroModel> saidaslucromodel = data.map((item) => saidaLucroModel.fromJson(item)).toList();

      // Definir os dados da resposta como a lista de InfooneModel
      apiResponse.data = saidaslucromodel;
    } else {
